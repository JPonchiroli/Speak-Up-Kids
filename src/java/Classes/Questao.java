...
public class Questao {
    ...
    public int consultarCodQuestao(int codAtividade, String numero_questao) {
        Connection con = Conexao.conectar();
        String sql = "select cod_questao ";
        sql += "FROM questao ";
        sql += "where cod_atividade = ? AND ";
        sql += "numero_questao LIKE ? ";
        int codQuestao = 0;
        try {
            PreparedStatement stm = con.prepareStatement(sql);
            stm.setInt(1, codAtividade);
            stm.setString(2, "%" + numero_questao + "%");
            ResultSet rs = stm.executeQuery();
            if (rs.next()) {
                codQuestao = rs.getInt("cod_questao");
            }
        } catch (SQLException ex) {
            System.out.println("Erro no Select Único: " + ex.getMessage() + "\nComando SQL : " + sql);
        }
